...
- [x] **Phase 58: 큐레이터 타임아웃 현실화(15초) 및 비상 폴백 도서 100% 실서지 메타데이터(ISBN/표지/쪽수) 완성형 바인딩**
  ...
  - **폴백 도서 100% 실서지 메타데이터(ISBN, 표지, 쪽수, 출판사, 장르) 완성형 바인딩 (`curator_node.py`, `national_library_client.py`)**:
    - `assemble_curated_books`를 비동기(`async`)로 개선하고, 비상 폴백 투입 시 `national_library_client.search_book`을 호출해 정식 13자리 ISBN, 교보문고 고화질 CDN 표지(458px), 실제 쪽수, 정식 출판사, KDC 장르가 완벽히 채워진 완성형 객체 반환.
    - 국립도서관 오프라인 카탈로그에 《모모》(미하엘 엔데, 비룡소, ISBN 9788949110271, 372쪽) 실서지 메타데이터를 공식 추가하여 비상 명작 8종 전원에 대한 100% 실서지 보장. 프론트엔드 도서 등록 폼(`/register`) 이동 시 정보 누락 0건 보장.
  - **단위 테스트 작성 및 AI 자가 검증 (Self-Validation)**:
...
- [x] **Phase 54: 도서 추천 포맷 하이재킹 방지 및 응답 시퀀스(공감 ➔ 사유 ➔ 헤딩) 강제화**
  - **도서 큐레이션 응답 시스템 프롬프트 순서(Sequence) 제약 추가 (`app/domain/graph/nodes.py`)**:
    ...
    - 공감과 사유 없이 다짜고짜 도서 등록 포맷/헤딩부터 던지는 '자판기식 출력(Format Hijacking)' 엄격 금지 지침 적용.
  - **서비스 공통 가드레일 네거티브 룰 보강 (`app/domain/guardrails/shared_rules.py`)**:
    - `SHARED_GUARDRAILS`의 도서 추천 항목에 감정/상황에 대한 위로와 사유 설명 없이 카드 마커/헤딩만 기계적으로 던지는 행위 금지 명시.
  - **자가 검증 및 테스트 100% 그린 패스 (Self-Validation)**:
    - `tests/unit/test_graph_handoff.py`: 큐레이션 도서 주입 시 시스템 프롬프트에 4단계 출력 순서 가이드라인이 누락 없이 주입되는지 검증하는 단위 테스트 추가.
    - 전체 **209개 단위 테스트 100% 통과** (`209 passed, 1 warning in 78.83s`), Ruff 린트/포맷 0 에러, Mypy 타입 체크 87개 소스 파일 0 에러 무결점 확인.

- [x] **Phase 55: 국립중앙도서관 분류 파이프라인 내 특정 도서명 하드코딩 제거 및 범용 키워드 SSOT 확립**
  - **`map_kdc_to_genre` 도서명 하드코딩 제거 (`app/infrastructure/national_library_client.py`)**:
    - `"오디세이아"`, `"오뒷세이아"`, `"그림의 힘"` 등 특정 도서명 하드코딩 안티패턴을 완전히 제거.
    - `"미술치료"`, `"심리치료"`, `"마음치유"`, `"소설"`, `"에세이"` 등 범용 도메인 키워드 및 `NON_CURATABLE_KEYWORDS` 필터링 유지.
  - **단위 테스트 및 AI 자가 검증 (Self-Validation)**:
    - `tests/unit/test_recommend_metadata.py`: `title="그림의 힘"` 단독 호출 테스트를 `title="미술치료 입문"` 등 범용 키워드 기반으로 갱신.
    - 전체 14개 단위 테스트 100% 그린 패스, Ruff 린트 0 에러 무결점 통과.
