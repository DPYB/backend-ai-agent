...
async def _run_persona_node(
    ...
    if curated_books:
        ...
        system_prompt += (
            ...
            "- [답변 출력 형식 준수 (시인성 극대화 및 마크다운 헤딩 규격)]:\n"
            "  * 화면에는 `### 📖 도서명` 헤딩 아래에 시스템 추천 도서 카드가 자동으로 예쁘게 렌더링됩니다.\n"
            ...
            "  * [추천 도서 헤딩 규격]: 추천 도서를 소개할 때는 반드시 `### 📖 {도서명}` 형식의 마크다운 3단계 헤딩만 별도의 줄에 명시하십시오. (헤딩에 다른 이모지나 부연 수식어를 붙이지 마십시오.)\n"
            "  * 본문에는 `저자: ...`, `사유: ...` 와 같은 딱딱한 메타데이터 목록이나 `---` 구분선을 따로 적지 마십시오. (화면 카드와 중복되어 시인성을 해칩니다.)\n"
            "  * 각 `### 📖 {도서명}` 헤딩 아래에 이 책을 권하는 따뜻한 이유와 감상을 당신의 고유 어조로 1~2문장 자연스럽게 서술하십시오.\n"
            "  [작성 구조 예시]:\n"
            "  (마음을 여는 사서의 다정한 서두 멘트 — 섹션 헤딩 없이 부드러운 평문으로 시작)\n\n"
            "  ### 📖 첫 번째 도서명\n"
            "  (이 책을 권하는 사서의 다정한 1~2문장 감상 및 위로 멘트)\n\n"
            "  ### 📖 두 번째 도서명\n"
            "  (이 책을 권하는 사서의 다정한 1~2문장 감상 및 위로 멘트)\n\n"
            "  (마무리 응원/작별 멘트)\n"
